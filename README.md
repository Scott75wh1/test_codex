...
## Web demo pronta nell'ambiente

Dopo l'avvio con `python app.py`, apri `http://localhost:5000` e trovi una demo già pronta:

- il telex di esempio è pre-caricato automaticamente,
- puoi cliccare `Genera QR` per vedere subito QR + campi compilati,
- puoi usare `Scarica QR PNG` per salvare il codice,
- puoi usare `Avvia scanner` per leggere un QR dalla camera.
