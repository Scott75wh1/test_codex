...
const App = () => {
  ...
  const createManual = async (type: 'fattura' | 'ddt') => {
    ...
    await refresh();
    alert(`${type.toUpperCase()} creato in archivio`);
  };

  const startPrint = async (docId: string) => {
    const selection = prompt('Opzioni stampa: fisica | pdf | slyce', 'slyce');
    if (!selection) return;
    if (selection === 'slyce') {
      const res = await window.api.runPrinter(docId);
      alert(res.ok ? `Pipeline completata (job ${res.jobId})` : `Errore: ${res.error}`);
      await refresh();
    }
    if (selection === 'pdf') alert('Selezionata opzione Salva PDF (simulata)');
    if (selection === 'fisica') alert('Selezionata stampante fisica (simulata)');
  };
  ...
  const archive = () => {
    const [supplier, setSupplier] = [ '', (v: string) => v ];
    void supplier; void setSupplier;
    return <table><thead><tr><th>N.</th><th>Tipo</th><th>Data</th><th>Fornitore</th><th>Cliente</th><th>Layout</th><th>Origine</th><th>Stato</th><th>Azioni</th></tr></thead><tbody>{state.documents.map((d: any) => {
      ...
      return <tr key={d.id}><td>{d.number}</td><td>{d.type}</td><td>{d.date}</td><td>{s?.companyName}</td><td>{c?.companyName}</td><td>{d.layoutId}</td><td>{d.creationMode}</td><td>{d.status}</td><td><button onClick={() => startPrint(d.id)}>Stampa</button>{d.pdfPath && <button onClick={() => window.api.openFile(d.pdfPath)}>PDF</button>}{d.jsonPath && <button onClick={() => window.api.openFile(d.jsonPath)}>JSON</button>}</td></tr>;
    })}</tbody></table>;
  };
  ...
  return <div className="app">
    ...
    <main>
      ...
      {active === 'Template Layout Documenti' && <section>
        ...
        }}>{state.layouts.map((l: any) => <option key={l.id} value={l.id}>{l.name}</option>)}</select><button onClick={() => alert(`Duplicazione config template ${s.preferredLayout} simulata`) }>Duplica config</button></div>)}
      </section>}
